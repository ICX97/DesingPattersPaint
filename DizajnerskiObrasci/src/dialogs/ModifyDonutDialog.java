...
public class ModifyDonutDialog extends JDialog{
	...
	public ModifyDonutDialog(Donut donut) {
		...
		txtX.setText(Integer.toString(donut.getCenter().getX()));
		panel.add(txtX);
		txtX.setColumns(10);

		txtY = new JTextField();
		txtY.setBounds(200, 55, 224, 25);
		txtY.setText(Integer.toString(donut.getCenter().getY()));
		panel.add(txtY);
		txtY.setColumns(10);

		txtR = new JTextField();
		txtR.setBounds(200, 95, 224, 25);
		txtR.setText(Integer.toString(donut.getR()));
		panel.add(txtR);
		txtR.setColumns(10);
		
		txtHr = new JTextField();
		txtHr.setBounds(200, 135, 224, 25);
		txtHr.setText(Integer.toString(donut.getHoleRadius()));
		panel.add(txtHr);
		txtHr.setColumns(10);
		...
		btnColor.setBackground(donut.getColor());
		panel.add(btnColor);
				...
				if (txtX.getText().trim().length() == 0) {
					...
					try {
						...
						if (x1 <= 0) {
							...
							donut.setCenter(new Point(x1, y1));
							donut.setR(r1);
							donut.setHoleRadius(hR1);
							donut.setInnerColor(btnInnerColor.getBackground());
