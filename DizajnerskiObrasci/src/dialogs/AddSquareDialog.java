...
public class AddSquareDialog extends JDialog{
	...
	public AddSquareDialog(int x, int y, DrawingFrame frame) {
				...
				if (txtSideLength.getText().trim().length() == 0) {
					...
					try {
						...
						if (side <= 0) {
							...
							square.setColor(frame.getSideFrame().getColorBtn().getBackground());
							square.setInnerColor(frame.getSideFrame().getInnerBtn().getBackground());
