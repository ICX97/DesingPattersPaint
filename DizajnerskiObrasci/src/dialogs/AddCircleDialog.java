...
public class AddCircleDialog extends JDialog {
	...
	public AddCircleDialog(int x, int y, DrawingFrame frame) {
				...
				if (txtRadius.getText().trim().length() == 0) {
					...
					try {
						...
						if (radius <= 0) {
							...
							circle.setR(radius);
							circle.setColor(frame.getSideFrame().getColorBtn().getBackground());
							circle.setInnerColor(frame.getSideFrame().getInnerBtn().getBackground());
