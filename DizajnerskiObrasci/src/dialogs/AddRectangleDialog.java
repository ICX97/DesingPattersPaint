...
public class AddRectangleDialog extends JDialog{
	...
	public AddRectangleDialog(int x, int y, DrawingFrame frame) {
				...
				if (txtWidth.getText().trim().length() == 0) {
					...
					try {
						...
						if (widthh <= 0) {
							...
							rectangle.setColor(frame.getSideFrame().getColorBtn().getBackground());
							rectangle.setInnerColor(frame.getSideFrame().getInnerBtn().getBackground());
