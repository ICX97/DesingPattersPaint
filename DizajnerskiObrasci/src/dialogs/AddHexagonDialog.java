...
public class AddHexagonDialog extends JDialog{
	...
	public AddHexagonDialog(int x, int y, DrawingFrame frame) {
				...
				if (txtRadius.getText().trim().length() == 0) {
					...
					try {
						...
						if (radius <= 0) {
							...
							hexagon.setY(y);
							hexagon.setR(radius);
							hexagon.setColor(frame.getSideFrame().getColorBtn().getBackground());
							hexagon.setInnerColor(frame.getSideFrame().getInnerBtn().getBackground());
